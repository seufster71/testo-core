...
public final class ErrorMsg {
	...
	public static void addMsg(Response response, String code, String message) {
		...
		if (response.getParams().containsKey(GlobalConstant.ERRORS)) {
			((List<Map<String, String>>) response.getParams().get(GlobalConstant.ERRORS)).add(msg);
		} else {
			List<Map<String,String>> messages = new ArrayList<Map<String,String>>();
			messages.add(msg);
			response.getParams().put(GlobalConstant.ERRORS, messages);
		}
	}
}
